...
function getFileData(fileName) {
  try {
    ...
    debugLog('getFileData', `Successfully read ${fileName}, size: ${data.length}`);
    return data;
  } catch (error) {
    ...
    case 'save': {
      ...
      try {
        ...
        const data = ArrayBuffer.isView(arrayBuffer)
          ? new Uint8Array(arrayBuffer.buffer.slice(arrayBuffer.byteOffset, arrayBuffer.byteOffset + arrayBuffer.byteLength))
